...
const handleMessageGPT = async (message: Message, prompt: string) => {
  try {
    ...
    cli.print(
      `[GPT] Answer to ${message.from}: ${response.text}  | OpenAI request took ${end}ms)`
    )
...
async function sendVoiceMessageReply(message: Message, gptResponse: any) {
  //* Get audio buffer
  cli.print(
    `[Speech API] Generating audio from GPT response "${gptResponse.text}"...`
  )
...
export { handleMessageGPT, sendVoiceMessageReply }
